...
**The software still implements the PRE-campaign vehicle** — 2S pack bands, syringe+stepper ballast, paddlewheel speed, every sensor on Pi GPIO, burn wire stubbed as v2. The bought boat (§2–§12 above) diverges on all of those. The repo's `docs/hardware.md` is now **this architecture's own document** (rewritten 2026-08-18/19, drawings embedded); its **§20 is the software-gap ledger**, and the changelog entry is `.specs/tasks.md` ("The parts are bought, and they are not the parts the software describes"). Integration status (2026-08-19): **the split is written, both ends** — ESP32 firmware (`firmware/brainstem/brainstem.ino`, Arduino C++, one library: Adafruit BNO08x; MS5837+INA219 raw-register) + Pi serial client (`api/brainstem.py`) + `RealHardware` reduced to two DRV8871 pairs (GPIO 23/24, 5/6) + 3S bands purged through mock, client and suites. A bare devkit on a breadboard plugged into ANY machine running the API lights the console (bench walk: `firmware/README.md` §2); announced bench mode + per-chip kill/revive rehearse the whole cannot-tell chain over the real cable. The remaining code debt is `docs/hardware.md` §20 (c_id stage for acks, client pump control shape, nav direction consumption, zone names on the wire, soundings wording, sonar ingest). ⚠ NEW HARDWARE FACT the firmware forced into the open: **the pump needs an H-bridge for direction** (one IRLZ44N can only fill) — a third DRV8871 on ESP32 pins 18/17 is the natural part; pins 16 (speed flow) and 32 (optional burn continuity) are also now assigned. Bench gates first (§15): pack recovery charge is URGENT; `PUMP_ML_PER_PULSE` needs the trickle test.
