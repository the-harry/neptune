"""Canal & River Trust hazard layers → GeoJSON on disk. BOOTSTRAP-TIME ONLY.
...
THE HONESTY DOCTRINE, APPLIED TO A DOWNLOAD
    ...
    pilot who reads the second as the first drives into a sluice. So:
      ...
      * a layer whose fetch failed part-way gets NO file. A truncated page is the exact
        shape of "no hazards here" and there is no way to tell them apart later, so the
        partial result is dropped and the failure is recorded instead. The serving side
        then reports it ABSENT, which is the true answer;
...
ON DISK
    <crt_dir>/<area>/<layer>.geojson       clipped FeatureCollection, carrying attribution
    <crt_dir>/<area>/<layer>.prov.json     that file's provenance, beside that file
    <crt_dir>/<area>/provenance.json       index: every layer on the card, every skip,
                                           every warning
    All three are written through _write_atomic, so a reader only ever sees a whole
    file. The index describes the CARD — including layers carried forward from an
    earlier fetch — and not merely what this run happened to download.
...
def safe_area_name(name: str) -> str | None:
    ...
    out of the data directory is not a name."""
    n = (name or "").strip()
