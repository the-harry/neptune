...
def _sim(args=None) -> int:
    ...
    print(f"samples={log.count}  path={sim.path_len:.0f}m  final depth={est.depth:.1f}m")
...
def _sample_from_row(row: dict) -> SensorSample:
    ...
    heading_deg comes from the RAW compass column when the log has one. The plain
    heading_deg column is the estimator's output, and under NAV_FILTER=filtered that is
    an already-filtered heading — feeding it back through the filter would score the
    filter against its own past opinion.
    """
    # `or`-style fallback via an explicit None check, not dict.get's default: the key
    # can be PRESENT and null (a vehicle whose compass said nothing), and a default only
    # fires on a missing key, so the naive version would hand pydantic a None.
    heading = row.get(RAW_HEADING_KEY)
    if heading is None:
        heading = row.get("heading_deg", 0.0)
    return SensorSample(
        t=row["t"],
        heading_deg=heading,
        depth_m=row.get("depth_m", 0.0),
        throttle=row.get("throttle", 0.0),
        encoder_m=row.get("encoder_m", 0.0),
        mag_cal=row.get("mag_cal", 3),
        ...
        gyro_z_dps=row.get("gyro_z_dps", 0.0),
        accel_fwd_ms2=row.get("accel_fwd_ms2", 0.0),
...
def load_replay_log(path: Path) -> ReplayLog:
    ...
    if origin_d:
        ...
        # independent — but say so, because the lat/lon printed for the track will be
        # off the coast of Africa and somebody will otherwise report that as a bug.
    ...
    if lut_id and lut_id != "default":
        try:
            ...
            notes.append(f"speed LUT {lut_id!r} could not be loaded ({exc}) — "
    ...
    if out.samples and not any(RAW_HEADING_KEY in r for r in rows):
        ...
        notes.append(
            ...
            + (". This dive was recorded under NAV_FILTER=filtered, so the replay is "
               ...
               "still sound)."))
    if out.samples and not any(s.speed_ms_measured is not None for s in out.samples):
        notes.append("the paddlewheel never reported in this log — the speed filter runs "
                     "on the LUT alone and the snag detector has nothing to see with")
...
def score(run: ReplayRun, log: ReplayLog) -> dict:
    ...
    return {
        ...
        "final_depth": run.states[-1].depth_m if run.states else 0.0,
...
def _print_run(sc: dict) -> None:
    x, y = sc["final_xy"]
    print(f"\n--- {sc['backend']} ---")
    print(f"  final          ({x:.1f}, {y:.1f}) m   depth {sc['final_depth']:.1f} m")
