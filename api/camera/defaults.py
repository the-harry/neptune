...
SETTINGS: tuple[Setting, ...] = (
    ...
    Setting(
        "power_saving", "Camera.Menu.PowerSaving",
        ("PowerSaving", "Camera.Menu.PowerSaving"), ("OFF",), CRITICAL,
    ...
    Setting(
        "power_off_delay", "Camera.Menu.PowerOffDelay",
        ("PowerOffDelay", "Camera.Menu.PowerOffDelay"), ("OFF",), CRITICAL,
        "Same class of failure as PowerSaving, on a separate timer.",
    ),
    Setting(
        "video_clip_time", "Camera.Menu.VideoClipTime",
        ("VideoClipTime", "Camera.Menu.VideoClipTime"),
        ("3MIN", "5MIN", "2MIN", "1MIN", "10MIN"), CRITICAL,
    ...
    Setting(
        "mtd", "Camera.Menu.MTD", ("MTD", "Camera.Menu.MTD"), ("OFF",), CRITICAL,
        "Motion-triggered recording. Drifting particulate underwater would trigger it "
        "continuously.",
    ),
    Setting(
        "gsensor", "Camera.Menu.GSensor", ("GSensor", "Camera.Menu.GSensor"), ("OFF",), CRITICAL,
        "Impact-triggered file locking. The sub bumps things routinely; locked files "
        "accumulate and fill the card.",
    ),
    Setting(
        "def_mode", "Camera.Menu.DefMode", ("DefMode", "Camera.Menu.DefMode"), ("VIDEO",), CRITICAL,
    ...
    Setting(
        "videores", "Camera.Menu.VideoRes", ("Videores",), ("1080P30",), QUALITY,
    ...
    Setting(
        "imageres", "Camera.Menu.ImageRes", ("Imageres",), ("12MP",), QUALITY,
    ...
    Setting(
        "hdr", "Camera.Menu.HDR", ("HDR", "Camera.Menu.HDR"), ("OFF",), QUALITY,
        "Underwater scenes are low-contrast, not high-contrast — HDR adds processing for "
        "little benefit and can look mushy.",
    ),
    Setting(
        "spot_meter", "Camera.Menu.SpotMeter", ("SpotMeter", "Camera.Menu.SpotMeter"),
        ("OFF",), QUALITY,
    ...
    Setting(
        "ev", "Camera.Menu.EV", ("EV", "Camera.Menu.EV"), ("EV0",), QUALITY,
    ...
    Setting(
        "status_lights", "Camera.Menu.StatusLights",
        ("StatusLights", "Camera.Menu.StatusLights"), ("OFF",), HULL,
        "Indicator LEDs are invisible inside a sealed hull and reflect off the inside of "
        "the port, flaring the recorded image.",
    ),
    Setting(
        "sound_indicator", "Camera.Menu.SoundIndicator",
        ("SoundIndicator", "Camera.Menu.SoundIndicator"), ("OFF",), HULL,
        "Beeps are inaudible inside a sealed hull, and keep any future hydrophone clean.",
    ),
    Setting(
        "looping_video", "Camera.Menu.LoopingVideo",
        ("LoopingVideo", "Camera.Menu.LoopingVideo"), ("OFF",), HULL,
        "Retain the whole dive rather than overwriting the earliest footage.",
    ),
    Setting(
        "flicker", "Camera.Menu.Flicker", ("Flicker", "Camera.Menu.Flicker"), ("50Hz",), HULL,
        "UK mains. Only matters under artificial light, but harmless and already correct.",
    ),
    Setting(
        "tv_system", "Camera.Menu.TVSystem", ("TVSystem", "Camera.Menu.TVSystem"), ("PAL",), HULL,
        "Matches 50Hz.",
    ),
    Setting(
        "timestamp", "Camera.Preview.MJPEG.TimeStamp", ("Camera.Preview.MJPEG.TimeStamp",),
        ("ACTIVE",), HULL,
    ...
    Setting(
        "preview_w", "Camera.Preview.H264.w", ("Camera.Preview.H264.w",), ("1280",), QUALITY,
    ...
    Setting(
        "preview_h", "Camera.Preview.H264.h", ("Camera.Preview.H264.h",), ("720",), QUALITY,
...
def awb_setting(white_lights_on: bool) -> Setting:
    ...
    if white_lights_on:
        ...
        why = ("White LEDs restore red at short range; a warm preset on top of them casts "
               "everything orange.")
    else:
        ...
        why = ("No lamps: the warmest preset available counteracts the blue-green cast that "
...
class Capabilities:
    ...
    def to_json(self) -> dict:
        return {
            "fw": self.fw, "write_names": self.write_names, "values": self.values,
            "ignored": sorted(set(self.ignored)), "rejected": sorted(set(self.rejected)),
        }
...
def load_caps(fw: str) -> Capabilities:
    ...
    if fw and raw.get("fw") and raw.get("fw") != fw:
        log.info("camera firmware changed (%s -> %s) — discarding the capability cache",
                 raw.get("fw"), fw)
        return Capabilities(fw=fw)
    return Capabilities(
        fw=raw.get("fw") or fw,
