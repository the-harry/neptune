...
class FakeInternet:
    ...
    def http_get(self, url: str, timeout: float = 20.0) -> bytes | None:
        ...
        try:
            ...
            if "nominatim" in url:
                return json.dumps({"address": {"waterway": "Birmingham Canal Navigations"},
                                   "name": "Birmingham Canal Navigations"}).encode()
            if "overpass" in url:
                return json.dumps({"elements": [{
                    "type": "way", "tags": {"waterway": "canal"},
                    "geometry": [{"lat": LAT, "lon": LON},
                                 {"lat": LAT + 0.001, "lon": LON + 0.001}]}]}).encode()
            with self._lock:
                self.tiles += 1
                n = self.tiles
            if self.fail_tiles_after is not None and n > self.fail_tiles_after:
                raise OSError(f"connection reset after {self.fail_tiles_after} tiles "
    ...
    def reachable(self, url: str, timeout: float = 4.0) -> tuple[bool, str]:
        ...
        return (True, f"{host}:443 answered") if self.online else (
            False, f"{host} does not resolve - there is no name service on this segment")
    ...
    async def download_hazards(self, area: str, bbox=None, progress=None) -> dict:
        ...
        for i, key in enumerate(self.LAYERS):
            ...
            if self.fail_from is not None and i >= self.fail_from:
                skipped.append({"layer_key": key, "skipped": "fetch-failed",
                                "why": "connection reset mid-page",
                                "note": "no file written on purpose - a partial layer "
            ...
            fetched.append({"layer_key": key, "file": path.name, "features": 1})
            if progress:
                await progress({"area": name, "state": "layer", "layer": key,
                                "n": i + 1, "of": len(self.LAYERS)})
        crtmod.provenance_path(name).write_text(json.dumps({
            "area": name, "bbox": list(bbox) if bbox else None,
            "finished": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
            "layers": fetched, "skipped": skipped, "warnings": []}))
        return {"ok": not skipped, "area": name, "layers": fetched, "skipped": skipped,
                "dir": str(out)}
...
_CREATE_NAMES = ("create_area", "create_area_for_point", "area_for_point",
                 "auto_create_area", "ensure_area", "ensure_area_for_origin")
...
def _call(fn, lat, lon, **extra):
    ...
    pool = {"lat": lat, "latitude": lat, "lon": lon, "lng": lon, "longitude": lon,
            "origin": origin, "o": origin, "point": (lat, lon), **extra}
...
def _metres(lat1, lon1, lat2, lon2) -> float:
    ...
    return math.hypot((lat2 - lat1) * m_lat,
                      (lon2 - lon1) * m_lat * math.cos(math.radians((lat1 + lat2) / 2)))
...
class AreaTestCase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self._set(data_dir=self.tmp, areas_dir=self.tmp / "areas",
                  dives_dir=self.tmp / "dives", speed_lut_dir=self.tmp / "speed_luts",
    ...
    def report(self, res: dict) -> dict:
        ...
        for attr in ("last_fetch", "fetch"):
            ...
            out[attr] = snap() if callable(snap) else (
...
class CreationTest(AreaTestCase):
    ...
    def test_a_launch_point_becomes_an_area_whose_box_contains_it(self):
        ...
        self.assertIsNotNone(fn, _searched(_CREATE_NAMES, areamod, svcmod) +
                             " - nothing in the repo can make an offline area, which is "
        ...
        self.assertTrue(w < LON < e and s < LAT < n,
                        f"the launch point {LAT},{LON} is not inside its own area's bbox {bb}")
        ...
        self.assertLess(off, 50.0,
                        f"the area's centre is {off:.0f} m from the launch point it was "
                        f"made for (bbox {bb})")
    ...
    def test_the_box_is_the_size_the_configuration_says_it_is(self):
        ...
        for label, got in (("north", north), ("east", east)):
            self.assertAlmostEqual(
                got, want, delta=max(60.0, want * 0.15),
    ...
    def test_the_new_area_carries_the_metadata_the_rest_of_the_system_reads(self):
        ...
        self.assertFalse(meta.get("present"),
        ...
        self.assertEqual(_state_of(meta), "absent",
    ...
    def test_the_same_launch_point_twice_leaves_one_area_not_two(self):
        ...
        self.assertEqual(_name_of(first, areas), _name_of(second, areas),
                         "the second call reported a different area from the first")
    ...
    def test_the_size_cap_refuses_before_it_starts_and_says_what_the_cap_is(self):
        ...
        with self.assertRaises(
                ...
                msg=f"a launch point asking for a {huge:.0f} m radius was accepted. "
                    ...
                    f"gigabytes over a phone hotspot at the water's edge.") as caught:
            _call(fn, LAT, LON, **extra)
        self.assertRegex(
            str(caught.exception), r"\d",
    ...
    def test_creating_an_area_resolves_no_hostname(self):
        ...
        self.assertEqual(self.net.urls, [],
                         f"creating an area fetched something: {self.net.urls[:3]}")
    ...
    def test_the_default_area_is_inside_the_tile_and_size_budgets(self):
        ...
        self.assertLessEqual(
            est["tiles"], nav_settings.sat_tile_cap,
        ...
        self.assertLessEqual(
            est["mb"], nav_settings.area_size_cap_mb,
            f"the default automatic area estimates {est['mb']} MB against a "
            f"{nav_settings.area_size_cap_mb} MB cap")
...
class AutoFetchTest(AreaTestCase):
    ...
    def test_setting_an_origin_makes_an_area_and_fetches_it(self):
        ...
        meta = areas[0]
        self.assertGreater(self.net.tiles, 0,
                           "an area was created and not one imagery tile was fetched")
        self.assertEqual(_state_of(meta), "present",
                         f"after a clean run the area reads {_state_of(meta)!r}; the "
                         f"imagery, the hazard layers and the centreline all landed")
        self.assertTrue(meta.get("present"),
                        f"the area's archive is not reported present: {meta}")
        self.assertTrue((nav_settings.areas_dir / f"{meta['name']}.mbtiles").exists(),
                        "no MBTiles archive on the card")
        self.assertTrue(meta.get("has_centreline"),
                        "no waterway centreline was fetched, so snapping has nothing to "
                        "snap to and the readiness gate cannot pass")
        self.assertTrue(self.haz.calls,
    ...
    def test_the_endpoint_returns_long_before_the_download_does(self):
        ...
        self.assertEqual(at_return, "downloading",
        ...
        self.assertEqual(_state_of(_area_meta()), "present",
    ...
    def test_a_re_run_skips_the_hazard_layers_already_on_the_card(self):
        self.set_origin()
        before = sorted(p.name for p in crtmod.area_dir(
            crtmod.safe_area_name(areamod.list_areas()[0]["name"])).glob("*.geojson"))
        ...
        self.assertEqual(len(reused), len(before),
                         f"after a re-run {len(reused)} of {len(before)} hazard layers "
                         f"are still readable on the card")
    ...
    def test_the_requests_are_made_one_at_a_time(self):
        ...
        self.assertEqual(self.net.max_live, 1,
                         f"{self.net.max_live} requests were in flight at once against "
                         f"somebody's free public service")
...
class InterruptedTest(AreaTestCase):
    ...
    def test_an_interrupted_fetch_leaves_nothing_that_reads_as_complete(self):
        ...
        self.assertIn(state, ("failed", "downloading", "absent", "interrupted"),
                      f"the area's state is {state!r}, which is none of "
                      f"{list(getattr(areamod, 'STATES', ()))}")
    ...
    def test_an_interrupted_fetch_fails_the_pre_dive_gate(self):
        ...
        self.assertIsNotNone(
            item, "the readiness check has no item covering whether the offline area is "
    ...
    def test_an_interrupted_fetch_leaves_enough_to_carry_on_from(self):
        ...
        self.assertEqual(_state_of(_area_meta()), "present",
                         "the resumed download did not finish the area")
...
class NoInternetTest(AreaTestCase):
    ...
    def test_with_no_internet_the_job_never_starts(self):
        ...
        self.assertEqual(
            [u for u in self.net.urls], [],
            f"requests were attempted anyway: {self.net.urls[:3]}")
    ...
    def test_the_absence_of_internet_is_reported_as_a_plain_fact(self):
        ...
        self.assertRegex(
            said, r"internet|offline|no network|not reachable|no route",
            ...
            "canal.")
        self.assertNotRegex(
            said, r"\b(traceback|exception|crash)\b",
            "the report of a perfectly ordinary condition reads as a crash")
    ...
    def test_the_launch_point_itself_still_works_with_no_internet(self):
        ...
        self.net.online = False
        self.set_origin()
        self.assertIsNotNone(self.svc.origin,
                             "with no internet the launch point was not even recorded")
...
class NoRealNetworkTest(AreaTestCase):
    ...
    def test_a_full_run_makes_no_request_this_suite_did_not_serve(self):
        ...
        for url in self.net.urls:
            self.assertRegex(url, r"^https?://",
                             f"a request went somewhere unexpected: {url!r}")
    ...
    def test_the_serving_path_still_answers_with_the_network_gone(self):
        ...
        block = svcmod._crt_layers(name)
        self.assertEqual(block.get("status"), "present",
                         f"the hazard index could not be served off the card: {block}")
