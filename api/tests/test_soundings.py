...
DIVES_PER_CELL = {(0.0, 10.0): ["dive-A", "dive-B"],
                  (20.0, 30.0): ["dive-A", "dive-B"],
                  (60.0, 70.0): ["dive-B"]}
...
# nothing whatsoever.
def dive_c_samples():
    return _profile(lambda t: blind(t, 10.0, 0.0, n=12),
                    lambda t: blind(t, 30.0, 0.0, n=12))
...
class SoundingsTestCase(unittest.TestCase):
    ...
    def _write_centreline(self) -> Path:
        coords = [[lon, lat] for lat, lon in
                  (to_latlon(x, y, ORIGIN.lat, ORIGIN.lon) for x, y in CENTRELINE_LOCAL)]
        gj = {"type": "FeatureCollection",
              "features": [{"type": "Feature", "properties": {"waterway": "canal"},
    ...
    def journal(self, dive_id: str, samples, *, with_ballast: bool = True) -> Path:
        ...
        for t, x, y, depth, ballast in samples:
            ...
            ns = NavState(t=t, lat=lat, lon=lon, depth_m=depth, heading_deg=90.0,
                          x_m=x, y_m=y, raw_lat=lat, raw_lon=lon, snapped=False,
                          range_m=math.hypot(x, y), payout_m=math.hypot(x, y) * 1.2)
            raw = None
            if with_ballast:
                raw = SensorSample(t=t, heading_deg=90.0, depth_m=depth, throttle=0.0,
                                   ballast_level=ballast, ballast_target=ballast,
    ...
    def spans(self, result) -> dict:
        ...
        for c in result["cells"]:
            ...
            self.assertNotIn(key, out, f"two cells claim the same stretch {key}: "
                                       f"{result['cells']}")
            out[key] = c
        return out
...
class KnownTouchesProduceKnownCells(SoundingsTestCase):
    ...
    def test_two_landings_in_one_cell_keep_the_deeper_and_count_as_two(self):
        ...
        self.assertAlmostEqual(cell[QUANTITY], 3.00, places=2,
                               msg=f"kept {cell[QUANTITY]} m of 3.00 and 1.50: {cell}")
        self.assertEqual(cell["contacts"], 2,
                         f"two separate landings in this cell were counted as "
                         f"{cell['contacts']}: {cell}")
        self.assertEqual(cell["samples"], 2 * HOLD_N,
                         f"{cell['samples']} samples from two {HOLD_N}-sample holds")
    ...
    def test_a_flat_depth_with_no_fill_is_not_a_landing(self):
        ...
        self.assertEqual(res["cells"], [],
                         f"a neutral-buoyancy hold produced soundings: {res['cells']}")
        self.assertRegex(
            str(res["reason"]), r"still filling|neutral buoyancy|syringe",
    ...
    def test_a_silent_stretch_inside_a_good_dive_creates_no_cell(self):
        res = self.build([self.journal("dive-A", dive_a_samples())])
        self.assertNotIn((70.0, 80.0), self.spans(res),
                         "the sensor was null for the stretch at 70 m and a cell "
...
class NoEvidenceSaysWhy(SoundingsTestCase):
    ...
    def test_a_journal_whose_depth_sensor_never_answered_produces_nothing(self):
        ...
        self.assertEqual(res["cells"], [],
                         f"a dive with no depth at all produced cells: {res['cells']}")

    def test_and_it_says_the_sensor_never_answered(self):
        res = self.build([self.journal("dive-C", dive_c_samples())])
        reason = str(res["reason"] or "")
        self.assertGreaterEqual(len(reason), 40,
                                f"the reason is barely a sentence: {reason!r}")
        self.assertRegex(reason, r"null on every sample|never answered",
                         f"the reason does not name what was missing: {reason!r}")
    ...
    def test_a_journal_with_no_ballast_column_gets_a_different_reason(self):
        ...
        no_ballast = self.build([self.journal("dive-D", dive_a_samples(),
                                              with_ballast=False)])
        silent = self.build([self.journal("dive-C", dive_c_samples())])
        self.assertEqual(no_ballast["cells"], [],
                         f"a journal with no ballast column produced soundings: "
                         f"{no_ballast['cells']}")
        self.assertRegex(str(no_ballast["reason"]), r"ballast",
                         f"the reason does not name the missing column: "
                         f"{no_ballast['reason']!r}")
        self.assertNotEqual(str(no_ballast["reason"]), str(silent["reason"]),
    ...
    def test_no_journals_at_all_is_a_third_reason_again(self):
        ...
        self.assertRegex(str(none["reason"]), r"no dive journals|nobody|never been",
                         f"{none['reason']!r}")
        self.assertNotEqual(str(none["reason"]).strip().lower(),
    ...
    def test_an_absent_centreline_is_absent_and_says_so(self):
        ...
        res = build_soundings([self.journal("dive-A", dive_a_samples())],
                              self.tmp / "no-such-area.geojson", cell_m=CELL_M)
        self.assertEqual(res["cells"], [])
        self.assertRegex(str(res["reason"]), r"ABSENT|does not exist",
                         f"a missing centreline was not reported as absent: "
                         f"{res['reason']!r}")
...
class BinningIsLongitudinal(SoundingsTestCase):
    ...
    def test_a_landing_far_off_the_axis_bins_by_distance_along_it(self):
        ...
        self.assertIn((0.0, 10.0), spans,
                      f"the landing 12 m off the axis did not bin at 5 m along: "
                      f"{sorted(spans)}")
        self.assertAlmostEqual(spans[(0.0, 10.0)][QUANTITY], 2.00, places=2)
        self.assertGreaterEqual(
            spans[(0.0, 10.0)]["offset_m_max"], 11.0,
    ...
    def test_cells_are_intervals_of_the_axis_on_cell_boundaries(self):
        res = self.build([self.journal("dive-A", dive_a_samples()),
                          self.journal("dive-B", dive_b_samples())])
        self.assertTrue(res["cells"], f"no cells: {res.get('reason')}")
        for c in res["cells"]:
            lo, hi = float(c["from_m"]), float(c["to_m"])
            self.assertAlmostEqual(hi - lo, CELL_M, places=3,
                                   msg=f"cell {(lo, hi)} is {hi - lo} m long: {c}")
            self.assertAlmostEqual(lo % CELL_M, 0.0, places=3,
                                   msg=f"cell {(lo, hi)} does not start on a boundary, "
                                       f"so two dives cannot share a cell by "
                                       f"construction: {c}")
            self.assertEqual(c["cell"], int(round(lo / CELL_M)),
                             f"the cell index disagrees with its own interval: {c}")
    ...
    def test_nothing_is_indexed_across_the_channel(self):
        ...
        for c in res["cells"]:
            across = [k for k in c
                      if re.fullmatch(r"row|col|column|j|iy|ix|grid_[xy]|across_m|"
                                      r"lateral_m|y_m", str(k), re.I)]
            self.assertFalse(across, f"a cell is indexed across the water as well as "
                                     f"along it ({across}): {sorted(c)}")
...
class TwoDivesCombine(SoundingsTestCase):
    ...
    def test_every_cell_names_the_dives_it_was_made_from(self):
        ...
        for span, want in DIVES_PER_CELL.items():
            ...
            self.assertIn(cell["deepest_from"]["dive_id"], want,
                          f"cell {span} does not say which dive the number itself came "
                          f"from: {cell['deepest_from']}")
    ...
    def test_the_per_dive_contributions_survive_the_merge(self):
        ...
        self.assertEqual(cell["samples"], sum(p["samples"] for p in per.values()),
    ...
    def test_re_reading_the_same_dive_does_not_double_count_it(self):
        ...
        self.assertEqual(self.depths(twice), self.depths(self.res),
                         "re-reading a journal changed the depths")
        for span, cell in self.spans(twice).items():
            self.assertEqual(cell["samples"], self.spans(self.res)[span]["samples"],
                             f"cell {span} counted dive-A twice: {cell}")
...
class TheLabelTravels(SoundingsTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.res = self.build([self.journal("dive-A", dive_a_samples()),
                               self.journal("dive-B", dive_b_samples())])
    ...
    def test_every_cell_carries_the_label_in_its_own_right(self):
        ...
        for c in self.res["cells"]:
            self.assertEqual(c.get("bound"), "lower",
                             f"a cell does not say what kind of number it is: {c}")
            self.assertIn(QUANTITY, c,
                          f"the depth is not under the name that says what it is "
                          f"({QUANTITY}): {sorted(c)}")
    ...
    def test_the_result_as_a_whole_explains_what_that_means(self):
        ...
        for key, pattern in (("means", r"AT LEAST"),
                             ("unsurveyed", r"[Aa]bsent is not shallow"),
                             ("datum", r"no vertical datum|surface")):
            self.assertRegex(str(self.res.get(key, "")), pattern,
                             f"result[{key!r}] does not say it: {self.res.get(key)!r}")
    ...
    def test_the_label_reaches_the_file_the_console_reads(self):
        ...
        self.assertTrue(doc.get("surveyed"), f"cells were written and surveyed is "
                                             f"{doc.get('surveyed')!r}")
        self.assertRegex(str(doc.get("unsurveyed", "")), r"[Aa]bsent is not shallow",
        ...
        for f in doc["features"]:
            ...
            self.assertRegex(str(p.get("what", "")), r"AT LEAST",
            ...
            self.assertEqual(f["geometry"]["type"], "LineString",
                             "a cell is a stretch of channel, not a dot somebody has "
                             "to guess the extent of")
    ...
    def test_an_empty_survey_still_writes_a_file_that_says_why(self):
        ...
        self.assertFalse(doc.get("surveyed"),
        ...
        self.assertTrue(doc.get("reason"),
                        f"an empty layer with nothing attached: a console reads that as "
                        f"'surveyed, nothing to say'. {doc}")
    ...
    def test_the_quantity_is_spelled_one_way_everywhere(self):
        ...
        out = write_geojson(self.res, self.tmp / "soundings.geojson")
        doc = json.loads(Path(out).read_text(encoding="utf-8"))
        self.assertEqual(soundings.QUANTITY, "lower_bound_m",
                         f"the quantity is named {soundings.QUANTITY!r}")
